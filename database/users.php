...
function user_exits($username, $conn)
{
  ...
  $result = $conn->query("SELECT `username` FROM `registration` WHERE `username` = '$username'");
...
{
  $username=sanitize($username,$conn);
  $result=$conn->query("SELECT `id` FROM `registration` WHERE `username` = '$username' ");
...
function login($username, $password)
{
  ...
  $result=$con->query("SELECT COUNT(`id`) AS count FROM `registration` WHERE `username`='$username' AND `password` = '$password' ");
